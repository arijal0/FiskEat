...
function NutritionCalculator({
  ...
  const renderProgressBar = (label: string, current: number, goal: number, unit: string, isLast = false) => {
    ...
    return (
      <div className={!isLast ? 'mb-4' : ''}>
        ...
        <div className="text-xs text-gray-500 mt-1">{percentage}% of goal</div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      />
      <div
        className="fixed bottom-20 right-6 w-96 h-[600px] bg-white rounded-lg shadow-2xl border border-gray-200 flex flex-col z-50"
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-4 text-center">
            <div className="text-3xl font-bold text-green-600">{totals.calories.toFixed(0)}</div>
            <div className="text-sm text-gray-600">Total Calories</div>
          </div>

          {/* Selected Items */}
          <div className="mb-4">
            <h4 className="text-sm font-semibold text-gray-700 mb-2">Selected Items</h4>
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-4">
            <h4 className="text-sm font-semibold text-gray-900 mb-3">Nutrition Breakdown</h4>
